...
// 当前账号看不了付费内容。
//
// 判定口径跟服务端 server/src/content.js 的 canSeePaid 逐条对齐——两处不
// 一致会让 UI 上的锁和实际能不能取到内容对不上（锁没显示、点进去却被 401；
// 或者显示了锁、其实账号明明能看）。试用期边界用同一个比较方向：
// trialEndsAt > now（服务端是 account.trial_ends_at > now），「严格大于」，
// 恰好等于 now 算已过期，不是「不晚于」。
//
