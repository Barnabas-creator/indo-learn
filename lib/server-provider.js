...
export function createServerProvider({ apiFetch, storage, cache, now = () => Date.now() }) {
  ...
  async function getIndex() {
    try {
      ...
      // 被明确拒绝（吊销/未激活/试用到期）才清会话；网络故障、限流都不算。
      const isRevoke = REVOKE_ERRORS.has(err?.message);
      if (isRevoke) { revokeReason = err.message; clear(); }
      // 无论哪种错误，先看看有没有缓存过的清单能兜底（哪怕是吊销后的最后一版，
      // 里面只有 id/tier/title，没有正文，兜底给它看不算泄露）。
      const cached = await cache.get('__index');
      if (cached) return cached;
      // 拒绝类错误、限流，原样抛给调用方——这两种调用方需要精确的错误码去区分文案。
      if (isRevoke || err?.message === 'rate_limited' || err?.message === 'not_found') throw err;
      // 剩下的统一按「网络故障/服务端自身问题」处理，跟 getUnit 用同一套约定，
      // 免得第 12 个任务接线 UI 时两边分支要分别处理。转换前打一条日志留住原始
      // 错误——服务端真会返回 server_error/no_content_key 这类「在线但坏了」的码，
      // 全被说成「还没下载过」会让排查很难。
      console.error('getIndex 失败且无缓存兜底：', err);
  ...
  async function getUnit(module, id) {
    ...
    try {
      ...
      // 别让调用方收到一个看不懂的 fetch 错误。转换前打一条日志留住原始错误，
      // 理由同 getIndex。
      console.error(`getUnit(${module}/${id}) 失败且无缓存兜底：`, err);
      throw new Error(OFFLINE);
    }
  }

  // 登出（不是被吊销）才清单元缓存：这是「设备换人」的边界。getUnit 是纯
  // cache-first，命中缓存直接返回、不会再走服务端鉴权，而 idb-cache.js 明确
  // 「明文存，不加密」——账号 A 看过的付费正文如果留在缓存里，账号 B 登录同一
  // 台设备后 getUnit 命中缓存就能直接读到，服务端鉴权全程不会被触发。
  //
  // 这段不能塞进 clear()：clear() 还被 getIndex/getUnit 里 REVOKE_ERRORS 命中时
  // 复用，那是同一账号被吊销/试用到期，仍需保留「最后一次缓存的清单目录」兜底
  // （只有 id/tier/title，没正文，风险低），塞进去会跟 getIndex 里紧跟的
  // cache.get('__index') 回退读取产生竞态，也会打断这个兜底。
  async function lock() {
    clear();
    await cache.clear();
  }
