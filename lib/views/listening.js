...
// —— 第二层：对话与听力的分类页 ——
export function renderAudioCats(root, { counts, open, back }) {
  const cats = [
    { id: 'dialogs', title: '情境对话', sub: '真实场景整段对话，配朗读', meta: `${counts.dialogs} 组` },
    { id: 'listening', title: '教材听力', sub: 'BIPA 官方教材录音 ＋ 听力题', meta: `${counts.listening} 段` },
  ];
  root.innerHTML = `
    <div class="stack">
      ...
      <div class="level-cards">
        ${cats.map((c) => `
          <button class="level-card ${c.meta.startsWith('0') ? 'soon' : ''}" data-id="${c.id}">
            <span class="level-badge">${esc(c.meta)}</span>
