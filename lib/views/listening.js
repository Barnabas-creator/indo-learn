...
// —— 第二层：对话与听力的分类页 ——
// 条数以前靠先把两份内容整包取回来数 length；现在内容按需取，为了一个数字在
// 这一层先发两次网络请求不划算，干脆不显示——比显示一个瞎猜的假数好。
export function renderAudioCats(root, { open, back }) {
  const cats = [
    { id: 'dialogs', title: '情境对话', sub: '真实场景整段对话，配朗读' },
    { id: 'listening', title: '教材听力', sub: 'BIPA 官方教材录音 ＋ 听力题' },
  ];
  root.innerHTML = `
    <div class="stack">
      ...
      <div class="level-cards">
        ${cats.map((c) => `
          <button class="level-card" data-id="${c.id}">
