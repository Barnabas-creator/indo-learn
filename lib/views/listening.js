...
import { LEVEL_NAMES } from './course.js';
...
//
// 听力按段切分之后（见 tools/content-units.mjs），清单摊平出来的每一条
// 只剩 meta.unitZh 这一个课名字符串，没有单独的「课 id」——分组直接拿
// unitZh 当 key，同一课的两段标题必然相同，不会撞车。
export function groupByUnit(items) {
  const units = [];
  const byTitle = new Map();
  for (const item of items ?? []) {
    let u = byTitle.get(item.unitZh);
    if (!u) {
      u = { title: item.unitZh, items: [] };
      byTitle.set(item.unitZh, u);
      units.push(u);
    }
    u.items.push(item);
  }
...
// —— 第三层：按单元列出每一段 ——
//
// items 是清单摊平出来的清单项（{id, tier, code, titleZh, unitZh, seconds}，
// 见 app.js 的 listenList 分支），不再是整段听力的正文数组——听力按段切分后
// （tools/content-units.mjs），正文（quiz/lines/phrases/vocab/tips）要点开
// 某一段才现取一次，列表层因此拿不到题数，只画得出时长。
export function renderListenList(root, items, { open, back }, expandedLevels = new Set()) {
  // 按分级折叠，跟课程页同一套（见 course.js renderCourseUnits）：A1 33 段 + A2 14 段
  // 摊成一条要滚很久。level 缺了按 A1——A1 那批内容文件没写 level 字段。
  // 展开状态只活在这次调用链的参数里，不记（本项目不做进度记录）。
  const levels = [...new Set((items ?? []).map((it) => it.level ?? 'A1'))];
  const group = (lv) => {
    const list = items.filter((it) => (it.level ?? 'A1') === lv);
    const expanded = expandedLevels.has(lv);
    return `
      <h3 class="level-head">
        <button class="level-toggle" type="button" data-level="${esc(lv)}" aria-expanded="${expanded}">
          <span class="level-caret" aria-hidden="true">${expanded ? '▾' : '▸'}</span>
          <span class="level-tag">${esc(LEVEL_NAMES[lv] ?? lv)}</span>
          <span class="level-meta">${list.length} 段</span>
        </button>
      </h3>
      ${expanded ? groupByUnit(list).map((u) => `
        ...
        <ul class="listen-list">
          ${u.items.map((it) => `
            <li><button class="listen-item" data-id="${esc(it.id)}">
              ...
              <span class="listen-meta">${mmss(it.seconds)}</span>
            </button></li>`).join('')}
        </ul>`).join('') : ''}`;
  };
  root.innerHTML = `
