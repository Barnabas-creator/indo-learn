...
// 注册成功但拿不到明文码（卖码模式）时显示：码已推给管理员，注册者本人看不到。
// 显示注册用的邮箱是为了让用户报给管理员时对得上号；管理员邮箱做成 mailto 链接，
// 方便在手机上直接点开发信。adminContact 从 lib/config.js 传入，视图本身不写死邮箱。
export function renderCodePending(root, { email, adminContact, onNext }) {
  const contact = escapeHtml(adminContact);
  root.innerHTML = shell('注册成功', '激活码已发送给管理员', `
      <p class="unlock-hint">请把你的注册邮箱发给管理员索取激活码：</p>
      <p class="unlock-hint"><a href="mailto:${contact}">${contact}</a></p>
      <p class="unlock-hint">注册邮箱：${escapeHtml(email)}</p>
      <p class="unlock-hint">激活码长期有效；如果管理员没收到通知，可在激活页点「重新申请激活码」。</p>
      <button class="next primary">我已拿到激活码，去激活</button>`);
  root.querySelector('.next').addEventListener('click', onNext);
}
