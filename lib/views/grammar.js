...
//
// 11.5：篇也是各自定价的，挂锁按条判——点击仍然照常触发 open(id)，是否跳登录
// 由 app.js 的 open 回调判 needsUnlock 决定，这里只管视觉提示，不拦点击
// （语法篇没有「准备中」状态，能出现在这里就能点）。
export function renderGrammarList(root, modules, { open, back, account }) {
  root.innerHTML = `
    <div class="stack">
      ...
      <ul class="grammar-list">
        ...
        <li><button class="grammar-item" data-id="${esc(m.id)}">
          ...
          ${needsUnlock(m, account) ? '<span class="grammar-lock" aria-label="需要登录">🔒</span>' : ''}
        </button></li>`).join('')}
      </ul>
    </div>`;
