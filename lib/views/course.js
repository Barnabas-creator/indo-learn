...
//
// 分级中文名——只在这一层展示，不影响 level 本身的取值（清单/挂锁逻辑照旧用 'A1'/'A2'）。
export const LEVEL_NAMES = { A1: 'A1 初级', A2: 'A2 初中级', B1: 'B1 中级', B2: 'B2 中高级' };

// 11.5：单元也是各自定价的，挂锁按条判——点击仍然照常触发 open(id)，是否跳
// 登录由 app.js 的 open 回调判 needsUnlock 决定，这里只管视觉提示，不拦点击。
//
// 折叠：A1 十个单元 + A2 十二个单元摊平成一长条，进来就要滚半天。改成按分级
// 折叠，进入时全收着，点标题才展开哪一级。展开状态不用记（本项目不做任何
// 进度记录，也不写 localStorage）——所以状态就活在 expandedLevels 这个参数
// 里：默认空集合＝全折叠，点一次折叠开关就带着更新过的集合重渲染一次自己，
// 状态只存在这次调用链的闭包里，App 一关/ 页面一换就没了，不需要额外清理。
export function renderCourseUnits(root, units, { open, back, account }, expandedLevels = new Set()) {
  ...
  root.querySelectorAll('.level-toggle').forEach((el) =>
    el.addEventListener('click', () => {
      ...
      renderCourseUnits(root, units, { open, back, account }, next);
    }));
