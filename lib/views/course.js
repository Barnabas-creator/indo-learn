...
// —— 第二层：单元列表 ——
// units 现在是清单条目（{ id, tier, title }，title 取自正文的 titleZh，见 app.js
// courseUnits 分支）。level/number/goal/课数都是正文字段，清单里没有，点进
// courseLessons 才按 id 取得到——A1/A2 分组、「准备中」占位这些原来靠这些字段
// 才画得出来的东西，现在一并去掉：清单本身只列服务端愿意给这个账号看的单元，
// 不在清单里的就是压根不显示，不是显示成「准备中」。
export function renderCourseUnits(root, units, { open, back }) {
  root.innerHTML = `
    <div class="stack">
      <div class="crumb"><button class="back">← 返回</button><span>课程 · BIPA</span></div>
      <p class="soon-note">印尼教育部官方教材 <em>Sahabatku Indonesia</em>。</p>
      <ul class="unit-list">
        ${units.map((u) => `
