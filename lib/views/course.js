...
// —— 第二层：单元列表，按 A1／A2 分组 ——
// 没填课的单元照样列出来，显示「准备中」、点不进去——跟单词包里没填词的包
// 是同一套约定：先让人看见整套课程有多少单元，再一个一个填。
export function renderCourseUnits(root, units, { open, back }) {
  ...
  const group = (lv) => {
    ...
    const ready = list.filter((u) => u.lessons.length).length;
    return `
      <h3 class="level-head">
        <span class="level-tag">${esc(lv)}</span>
        <span class="level-meta">${ready === list.length ? `${list.length} 个单元` : `已开放 ${ready} / ${list.length} 个单元`}</span>
      </h3>
      <ul class="unit-list">
        ${list.map((u) => {
          const on = u.lessons.length > 0;
          return `
          <li><button class="unit-item${on ? '' : ' soon'}" data-id="${esc(u.id)}" ${on ? '' : 'disabled'}>
            ...
            <span class="unit-count">${on ? `${u.lessons.length} 课` : '准备中'}</span>
          </button></li>`;
        }).join('')}
      </ul>`;
  };
  ...
  root.querySelectorAll('.unit-item:not([disabled])').forEach((el) =>
