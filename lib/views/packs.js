    ...
    <div class="stack">
      ...
      <div class="level-cards">
        ${levels
          .map(
            (lv) => `
          <button class="level-card ${lv.ready ? '' : 'soon'}" data-id="${lv.id}">
            <span class="level-badge">${lv.ready ? '开放' : '准备中'}</span>
            <span class="level-title">${lv.title}</span>
            <span class="level-sub">${lv.subtitle}</span>
            <span class="level-meta">${counts[lv.id]} 个主题</span>
          </button>`,
          )
          .join('')}
      </div>
...
// —— 第三层：某一级的主题包网格 ——
export function renderPackGrid(root, { levelTitle, packs, ready, open, back }) {
  const grid = packs
    .map(
      (p, i) => `
      <button class="pack-card" data-i="${i}" ${ready ? '' : 'disabled'}>
  ...
  root.innerHTML = `
    <div class="stack">
      ...
      ${ready ? '' : '<p class="soon-note">这一级的词条正在准备中，先看看会有哪些主题。</p>'}
