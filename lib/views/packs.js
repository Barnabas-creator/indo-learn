...
// —— 第三层：某一级的主题包网格 ——
// account：{ status, trialEndsAt }，11.5 起 needsUnlock 要按账号状态判定要不要
// 挂锁（不再是「tier === 'paid' 就一定锁」），不传就退化成「当匿名处理」——
// 见 catalog-view.js 的 canSeePaidLocally 对 account 的默认值处理。
export function renderPackGrid(root, { levelTitle, packs, open, back, account }) {
  // 开放与否现在看清单（p.open，见 lib/catalog-view.js 的 packsWithStatus），不再看
  // 词条——词条要点进包才按需去取，网格这一层压根不知道有几个词，别猜。
  const openCount = packs.filter((p) => p.open).length;
  const grid = packs
    .map((p, i) => {
      const ready = p.open;
      // 挂锁：清单里有这个包（能点）但账号看不了付费内容——点这张卡片时
      // app.js 的 open 回调会直接跳登录（不进详情页再等 401），这里只管
      // 显示锁标，不拦点击（拦截逻辑不在视图层，见 app.js 的 packGrid 分支）。
      const locked = needsUnlock(p, account);
      // 编号按包在这一级里的位置算，不是 id 里的数字——「准备中」的包也占位，
      // 这样「我背到第 37 包」跟屏幕上看到的永远对得上。
      const no = String(i + 1).padStart(2, '0');
      return `
      <button class="pack-card ${ready ? '' : 'soon'}" data-i="${i}" ${ready ? '' : 'disabled'}>
        <span class="pack-no">${no}</span>
        ...
        ${ready ? '' : '<span class="pack-count">准备中</span>'}
      </button>`;
    })
    .join('');
  root.innerHTML = `
    ...
    root.innerHTML = `
      <div class="card-view">
        <div class="crumb">
          ...
          <span class="card-progress">${pack.no ? pack.no + ' ' : ''}${pack.title} · ${index + 1}/${pack.words.length}</span>
        </div>
