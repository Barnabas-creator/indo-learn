...
本站没有构建步骤，用 Pages 的「从分支部署」即可，不需要 Actions 工作流。
（`gh` 的默认 token 也没有 `workflow` scope，推带工作流的提交会被 GitHub 拒绝；
要加 CI 得先跑 `gh auth refresh -s workflow`。）

```bash
cd ~/印尼语学习
gh repo create indo-learn --public --source=. --remote=origin \
  --description "印尼语学习 · 单词包/场景对话/语法学习 离线 PWA"
git push -u origin main
gh api -X POST repos/<用户名>/indo-learn/pages \
...
cd ~/印尼语学习
node --test tools/*.test.mjs
git add README.md index.html data assets
...
| 生成 1000 词条 | Task 8 |
| 生成 10 组对话 | Task 9 |
| GitHub Pages 部署（分支部署，无工作流） | Task 15 |
